/**
 * GHB-188 — DELETE /api/connected-apps/[id]
 *
 * Revokes an OAuth token by setting revoked_at = now().
 * Ownership enforced in revokeConnectedApp via user_id equality on the DB query.
 */
...
export async function DELETE(
  ...
  const result = await revokeConnectedApp(getServiceRoleClient(), { id, user_id });
